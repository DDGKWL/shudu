/*
*Copyright [2023] <Copyright 2011972 2012624>
*/
    ...
    do {
        do {
            do {
                    ...
                    for (int i = 0; i < 9; i++) {
                        for (int j = 0; j < 9; j++) {
                            int tmp = static_cast<int>
                                (sudoku_template[row_list[i]][j] - '0') - 1;
                ...
                } while (next_permutation(number_list, number_list + 9));
                // exchange in number_list
            } while (next_permutation(row_list + 6, row_list + 9));
            // exchange row 7~9
        } while (next_permutation(row_list + 3, row_list + 6));
        // exchange row 4~6
    } while (next_permutation(row_list, row_list + 3));
    // exchange row 1~3
    return 1;
}
...
bool canFill(int x, int y, int number) {
    if (row_record[x][number] == 0
        && col_record[y][number] == 0
        && block_record[getBlockIndex(x, y)][number] == 0)
        return true;
    else
        return false;
}
...
int solveSudokuGame(char* questpath) {
    ...
    while (!feof(sg)) {
        ...
        for (int i = 0; i < num_blank; i++) {
            blank_arr[i].count = col_num[blank_arr[i].y]
                + row_num[blank_arr[i].x]
                + block_num[getBlockIndex(blank_arr[i].x, blank_arr[i].y)];
        }
...
bool fillNumberNotN(int index, int size, int n, int x, int y, int tmp[9][9]) {
    if (index != size) {
        for (int i = 1; i < 10; i++) {
            if ((blank_arr[index].x == x)
                && (blank_arr[index].y == y) && (i == n)) {
                // cout << i << endl;
                continue;
            }
...
void creat_blank_range(int min, int max) {
    std::random_device rd;
    std::mt19937 rng(rd());
    std::uniform_int_distribution<int> dist(1, 100);
    int round = (dist(rng) % (max - min + 1)) + min;
...
int generateRange(int n, int min, int max) {
    generateSudokuEndGame(n);
    char content[1000];
    FILE* sg = fopen("./sudoku_endgame.txt", "r");
    if (sg == NULL) {
    ...
    FILE* result = fopen("./sudoku_range.txt", "wt");
    fwrite(save_arr, sizeof(char), save_num, result);
    cout << "finish creat " << n << " range game" << endl;
    fclose(result);
    fclose(sg);
    return 0;
}
...
int generateSudokuGame(int n) {
    ...
    FILE* result = fopen("./sudoku_game.txt", "wt");
    fwrite(save_arr, sizeof(char), save_num, result);
    cout << "finish generate sudoku game" << endl;
    fclose(result);
    fclose(sg);
    return 0;
}
